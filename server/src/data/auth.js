...
export async function findById(id) {
    return db.query(`SELECT id, mongo_id AS \"mongoID\", email, username, image_url AS \"imageURL\", status_msg AS \"statusMsg\", nickname, password 
...
export async function createUser(user) {
    const {email, nickname, username, password} = user;
    return await db.query("INSERT INTO users (email, nickname, username, password) VALUES ($1, $2, $3, $4) RETURNING id",
...
export async function resetPassword(user) {
    ...
    return db.query("UPDATE users SET password=$1 WHERE username=$2", [newPassword,username])
...
export async function findFriendRequest(user_id,friend_id) {
    return db.query(`SELECT * FROM friend_requests WHERE from_user_id=$1 AND to_user_id=$2`
                    ,[user_id,friend_id])
        .then((result)=>result.rows[0])
        .catch((err)=>console.log(err));
}
...
export async function deleteFriend(user_id,friend_id) {
    return db.query(`DELETE FROM friendships WHERE (user_id=$1 AND friend_id=$2) OR (user_id=$2 AND friend_id=$1)` 
                    ,[user_id,friend_id])
        .then((result)=>result.rowCount)
        .catch((err)=>console.log(err));
}
...
export async function createUserProfile(userID,nickname) {
    const newUser = await User({userID,nickname}).save();
    await db.query("UPDATE users SET mongo_id=$1 WHERE id=$2", [newUser.id,userID])
    return newUser
}
